...
require_once 'classes/AttributionClasseManager.php';

$db = new Database();
$manager = new AttributionClasseManager($db);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['ajouter'])) {
        ...
        $success = "Attribution ajoutée avec succès.";
    }

    if (isset($_POST['modifier'])) {
        $id = intval($_POST['id']);
        $manager->modifier($id, $enseignant_id, $classe_id, $matiere_id);
        header("Location: attribution_classes.php?updated=1");
        exit;
    }
}
...
if (isset($_GET['supprimer'])) {
    ...
    header("Location: attribution_classes.php?deleted=1");
    exit;
}
